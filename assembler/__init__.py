...
from .parser import assemble_lines, AssemblyError
from .output import to_hex_string


def _run(source: str, base_dir: str = ".") -> list[tuple[int, list[int]]]:
    processed, defines = preprocess(source, base_dir)
    tokens = tokenize(processed)
    return assemble_lines(tokens, defines)


def assemble(source: str, base_dir: str = ".", fmt: str = "bytes") -> bytes | str:
    coded = _run(source, base_dir)
...
def assemble_file(path: str, fmt: str = "bytes") -> bytes | str:
    ...
    with open(path, "r") as f:
        source = f.read()
    return assemble(source, base_dir=base_dir, fmt=fmt)


def assemble_list(source: str, base_dir: str = ".") -> str:
    processed, defines = preprocess(source, base_dir)
    tokens = tokenize(processed)
...
def assemble_list_file(path: str) -> str:
    ...
    with open(path, "r") as f:
        source = f.read()
    return assemble_list(source, base_dir)
